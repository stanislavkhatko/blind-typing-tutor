...
export const Game: React.FC<GameProps> = ({
  ...
  const handleInput = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (val.length > input.length) {
      ...
      if (val.length > 0) {
        setAccuracy(
          Math.max(0, ((val.length - currentErrors) / val.length) * 100)
        );
      } else {
        setAccuracy(100);
      }
  ...
  return (
    ...
    >
      ...
      {/* Mode and Learning Language Selectors - Above Text Display */}
      <div className="w-full max-w-4xl mb-4 flex gap-3 justify-center items-center">
        ...
        <div className="relative group">
          ...
          <select
            ...
            title={gameTranslations.learningMode}
          >
            <option value="practice">{gameTranslations.practice}</option>
            <option value="beginner">{gameTranslations.beginner}</option>
            <option value="custom">{gameTranslations.custom}</option>
          </select>
        ...
        <div className="relative group">
          ...
          <select
            ...
            onChange={(e) => {
              ...
              setLearningContentType(
                contentType as "words" | "phrases" | "programming"
              );
              setLearningLanguage(lang as LanguageCode);
            }}
            className="appearance-none bg-white dark:bg-gray-800 border-2 border-indigo-200 dark:border-indigo-800 text-gray-900 dark:text-white py-2.5 pl-10 pr-8 rounded-lg cursor-pointer focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 focus:outline-none text-sm font-semibold transition-all shadow-sm hover:shadow-md"
            title={gameTranslations.learningLanguage}
          >
            <optgroup label={gameTranslations.words}>
        ...
        >
          ...
          {text.split("").map((char, index) => {
            ...
            if (index < input.length) {
              ...
              color =
                "text-gray-900 dark:text-white border-b-2 border-blue-500 dark:border-blue-400"; // Cursor
            }
            return (
      ...
      <div className="relative w-full max-w-4xl">
        {/* Keyboard Layout Selector - Above Keyboard */}
        {showKeyboard && (
        ...
        <div className="absolute top-0 right-0 flex gap-2 mb-2 z-10">
          ...
          <button
            ...
            title={
              soundEnabled
                ? gameTranslations.soundOn
                : gameTranslations.soundOff
            }
            aria-label={
              soundEnabled
                ? gameTranslations.soundOn
