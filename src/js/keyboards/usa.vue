...
export default {
  ...
  mounted() {
    this.timestamps.unshift(this.getTimestamp());
  ...
  methods: {
    ...
    getRandomKey() {
      return this.keys[this.getRandomNumber(0, this.keys.length - 1)];
    },
    targetRandomKey() {
      const key = document.getElementById(this.getRandomKey());
    ...
    handleKeyUp(event) {
      ...
      if (keyPressed === highlightedKey.innerHTML) {
        this.timestamps.unshift(this.getTimestamp());
        const elapsedTime = this.timestamps[0] - this.timestamps[1];
