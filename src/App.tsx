...
function App() {
  ...
  const availableLayouts = allLayouts.filter(
    (layout) => POPULAR_LAYOUT_IDS.includes(layout.id) || layout.id === settings.layoutId
  ).sort((a, b) => {
  ...
  if (isMobile) {
    return <MobileMessage title={t.title} />;
  }

  return (
    <div className={`min-h-screen flex flex-col transition-colors duration-300 ${settings.darkMode ? "dark:bg-gray-900" : "bg-gray-50"}`}>
      <Header
        title={t.title}
      ...
      <main className="flex-grow pt-20">
        <Game
          ...
          onToggleKeyboard={() => settings.setShowKeyboard(!settings.showKeyboard)}
          onToggleHands={() => settings.setShowHands(!settings.showHands)}
          onToggleColors={() => settings.setShowColors(!settings.showColors)}
          onToggleCorrection={() => settings.setCorrectionMode(!settings.correctionMode)}
