...
test.describe("Blind Typing Tutor E2E Tests", () => {
  ...
  test("should load the application and display keyboard", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="keyboard-toggle-button"]')
    ).toBeVisible();
  ...
  test("should calculate errors correctly", async ({ page }) => {
    ...
    const errorsElement = page
      .locator('[data-testid="errors-stat"]')
  ...
  test("should toggle color zones", async ({ page }) => {
    ...
    const colorToggle = page.locator(
  ...
  test("should process keyboard input correctly for different layouts", async ({
    page,
  }) => {
    // Wait for text to be generated
    await page.waitForFunction(
    ...
    const textToType = textContent!
      .trim()
      .substring(0, Math.min(3, textContent!.trim().length));
  ...
  test("should handle correction mode", async ({ page }) => {
    ...
    const correctionToggle = page.locator(
